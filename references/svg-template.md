...
手机上看公众号，正文图显示宽度约 375px。SVG 画布宽 680、输出 1080px，
也就是说 SVG 里的 20px 字，到了手机上只剩约 7px —— 这已经是可读下限。

低于 16px 的字，缩到手机上就是一团模糊。示意图宁可少画几项、多分一张图，
也不要缩字号去塞内容。
...
3. **`<text>` 的 `y` 是基线。** 垂直居中要塞进高 `h` 的块时用 `y = 块y + h/2 + 4`
   （20px 字号用 +7）。别用 `dominant-baseline`，librsvg 支持不稳定。
